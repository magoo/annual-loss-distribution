  ...
  const description = $derived.by(() => {
    const bySection = activeSection === 'frequency'
      ? {
          ...
          scenario: 'Scenario Mode uses the scenarios below to run up to 10,000 simulated years. In each year, it decides which scenarios happen and how often, then totals incidents across scenarios.',
        }
      : {
          ...
          scenario: 'Scenario Mode uses the scenarios below to run up to 10,000 simulated years. It samples incident counts and costs from those scenario assumptions to build the cost distribution.',
        };
