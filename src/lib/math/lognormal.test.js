...
import { computeLognormal, fitLognormal } from './lognormal.js';
...
describe('computeLognormal', () => {
  describe('P50/P95 fidelity', () => {
    ...
    for (const { p50, p95, label } of cases) {
      it(`CDF is calibrated at P50 and P95 (${label}: p50=${p50}, p95=${p95})`, () => {
  ...
  describe('numerical stability', () => {
    it('returns finite, strictly increasing x values for extreme but valid quantiles', () => {
      const result = computeLognormal({ p50: 0.001, p95: 1000 });
      expect(result).not.toBeNull();
...
describe('fitLognormal', () => {
  it('recovers mu and sigma from exact P50/P95 lognormal quantiles', () => {
    const mu = 3.0;
    const sigma = 1.5;
    ...
    const result = fitLognormal(p50, p95);
